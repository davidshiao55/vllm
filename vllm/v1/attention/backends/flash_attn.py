...
class FlashAttentionImpl(AttentionImpl):
    ...
    def forward(
        ...
        if head_split is not None:
            ...
            prefetch_kv_heads = int(head_split.prefetch_kv_heads)
            if prefetch_kv_heads > 0:
                if head_split.kv_prefetch is None:
                    raise RuntimeError(
                        "COTS head-split KV prefetch requested P heads "
                        "without a prefetch descriptor"
                    )
                from vllm.model_executor.offloader import get_offloader

                attention_inputs = getattr(
                    get_offloader(),
                    "head_split_kv_prefetch_attention_inputs",
                    None,
                )
                if attention_inputs is None:
                    raise RuntimeError(
                        "COTS head-split KV prefetch attention has no offloader hook"
                    )
                (
                    prefetch_key_cache,
                    prefetch_value_cache,
                    prefetch_block_table,
                ) = attention_inputs(head_split.kv_prefetch)
                prefetch_query_start = int(head_split.prefetch_query_start)
                prefetch_query_heads = int(head_split.prefetch_query_heads)
                prefetch_query_end = prefetch_query_start + prefetch_query_heads
                prefetch_query = query[
                    :num_actual_tokens,
                    prefetch_query_start:prefetch_query_end,
                    :,
                ].contiguous()
                prefetch_output = torch.empty_like(prefetch_query)
                prefetch_descale_shape = (
                    int(head_split.cpu_seq_lens.shape[0]),
                    prefetch_kv_heads,
                )
                flash_attn_varlen_func(
                    q=prefetch_query,
                    k=prefetch_key_cache,
                    v=prefetch_value_cache,
                    out=prefetch_output,
                    cu_seqlens_q=attn_metadata.query_start_loc,
                    max_seqlen_q=(
                        1 if head_split.is_decode else attn_metadata.max_query_len
                    ),
                    seqused_k=attn_metadata.seq_lens,
                    max_seqlen_k=attn_metadata.max_seq_len,
                    softmax_scale=self.scale,
                    ...
                    alibi_slopes=None,
                    window_size=None,
                    block_table=prefetch_block_table,
                    softcap=self.logits_soft_cap,
                    scheduler_metadata=None,
                    fa_version=self.vllm_flash_attn_version,
                    q_descale=layer._q_scale.expand(prefetch_descale_shape),
                    k_descale=layer._k_scale.expand(prefetch_descale_shape),
                    v_descale=layer._v_scale.expand(prefetch_descale_shape),
                    num_splits=attn_metadata.max_num_splits,
                    s_aux=None,
                )
                output[
