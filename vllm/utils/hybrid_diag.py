...
"""Env-gated diagnostic helpers for Hybrid attribution.

Split diagnostic gates:

* `VLLM_HYBRID_NVTX=1` enables NVTX `range_push`/`range_pop` pairs.
* `VLLM_HYBRID_COUNTERS=1` enables cheap C++/Python counters.
...
across `hybrid.py`, `hybrid_ops.py`, `gpu_model_runner.py`,
...
# Single env reads at first import. `ENABLED` is the "some Hybrid diagnostic is
# active" umbrella for callers that only need a coarse gate.
NVTX_ENABLED = _flag("VLLM_HYBRID_NVTX")
COUNTERS_ENABLED = _flag("VLLM_HYBRID_COUNTERS")
...
def nvtx_range(name: str):
    ...
    just a `yield`. Use at boundaries the Hybrid diagnostic timeline
