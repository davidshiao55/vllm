...
def _reference_qwen_suffix_attention(
    query: torch.Tensor,
...
def test_cots_qwen_suffix_attention_matches_reference(
    block_size: int,
    ...
    output_lse = torch.empty(28, batch, dtype=torch.float32)
    scale = 128**-0.5
    ...
    ref_output, ref_lse = _reference_qwen_suffix_attention(
        query,
        key_cache,
        value_cache,
        block_table,
...
def test_cots_qwen_suffix_attention_rejects_out_of_range_block_id() -> None:
    dtype = torch.bfloat16
...
def test_cots_qwen_scatter_suffix_kv_matches_index_assignment() -> None:
